...
const ArticleTimeline: React.FC<ArticleTimelineProps> = ({ events, isLoading }) => {
  if (isLoading) {
    return (
        <div className="my-12">
            <h3 className="text-2xl font-extrabold mb-8 border-l-4 border-deep-red pl-4 flex items-center gap-2">
                <SparklesIcon className="text-gold"/> Interactive Timeline
            </h3>
      ...
      <div className="relative border-l-4 border-slate-200 dark:border-slate-700 ml-4">
        {events.map((event, index) => (
          <div key={index} className="mb-8 pl-8 relative">
            <div className="absolute -left-[18px] top-1 bg-gold w-8 h-8 rounded-full flex items-center justify-center text-white border-4 border-white dark:border-navy">
